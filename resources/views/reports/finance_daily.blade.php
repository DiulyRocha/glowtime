...
<div class="bg-white rounded-xl shadow p-6">
    ...
    {{-- 🔹 Filtro por data --}}
    <form method="GET" action="{{ route('reports.finance.daily') }}" class="mb-6 flex flex-wrap items-end gap-3">
        <div>
            <label for="date" class="block text-sm text-gray-600 font-semibold">Selecione o dia:</label>
            <input 
                type="date" 
                id="date" 
                name="date"
                value="{{ \Carbon\Carbon::hasFormat($date, 'd/m/Y') 
                    ? \Carbon\Carbon::createFromFormat('d/m/Y', $date)->format('Y-m-d') 
                    : ( \Carbon\Carbon::hasFormat($date, 'Y-m-d') 
                        ? $date 
                        : \Carbon\Carbon::parse($date)->format('Y-m-d') ) }}"
                class="border rounded px-3 py-2 focus:ring-pink-500 focus:border-pink-500">
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-gray-100 text-gray-700 p-4 rounded-lg text-center shadow-sm">
            ...
            <p class="text-sm uppercase">Data</p>
        </div>
    </div>
    ...
    {{-- 🔹 Navegação para outros relatórios --}}
    ...
    <p class="text-xs text-gray-500 mt-6">
        * Este relatório considera apenas os agendamentos com status de pagamento <strong>"Pago"</strong>.
    </p>
</div>
@endsection
