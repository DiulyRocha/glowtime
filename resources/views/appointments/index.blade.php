...
<table class="w-full mt-4 bg-white rounded shadow">
    ...
    <tbody>
        ...
        <tr class="border-t">
            ...
            <td>{{ \Carbon\Carbon::parse($a->end_time)->format('d/m/Y H:i') }}</td>
            <td>R$ {{ number_format($a->price_cents / 100, 2, ',', '.') }}</td>
            <td>{{ ucfirst($a->status) }}</td>
            <td>{{ $a->payment_status == 'paid' ? 'Pago' : 'Pendente' }}</td>
            <td>
                <form action="{{ route('appointments.cancel',$a) }}" method="POST" class="inline">
                    @csrf
                    @method('PUT')
                    <button class="text-red-500">Cancelar</button>
                </form>
                <form action="{{ route('appointments.markPaid',$a) }}" method="POST" class="inline">
                    @csrf
                    @method('PUT')
